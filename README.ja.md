...
現在は **1.1.0 Windows デスクトップ版**です。[リリースノート](docs/releases/v1.1.0.md)をご覧ください。取り込んだコードを実行するツールではありません。解析の成功は、プログラムが正しく動くことを保証しません。
...
Windows 10/11 x64 用の `Who-Is-JSON-1.1.0-Windows-x64-Setup.exe` を [Releases](https://github.com/Laurent150/Who-Is-JSON/releases/latest) からダウンロードしてください。Node.js と Python を同梱し、デスクトップにショートカットを作成します。インストーラーは未署名です。クラウドログイン・同期・AI には接続可能なネットワークが必要です。以下はソースから実行する手順です。
...
ブラウザーで **http://127.0.0.1:43127** を開きます。停止はターミナルで `Ctrl+C`。ポートは `CODELINGO_PORT` で変更できます。アプリはローカルホストでのみ待ち受けます。スクリーンショット機能とフローティングウィンドウは Windows 専用です。Windows 用 EXE インストーラーは Releases から取得できます。ソース ZIP には開発環境が必要です。
...
未ログイン時のお気に入りは現在のブラウザーに保存されます。GitHub でログインすると、アカウントのお気に入りと関連ソースコードをクラウドに同期できます。既存のローカル保存は自動でアップロードされません。既定の `http://127.0.0.1:43127/` 用の公開接続設定を同梱しているため、`.env` の設定は不要です。ネットワークによってクラウドに接続できない場合があり、VPN 不要の接続は保証しません。提供枠がある場合、ログイン後に DeepSeek Flash の無料お試しへ自動接続します。個人とサービス全体に利用上限があります。自分の AI サービスも設定できます。[AI お試しの詳細](docs/AI_TRIAL.md)。[詳細](docs/CLOUD_ACCOUNTS.md)。
