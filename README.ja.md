...
未ログイン時のお気に入りは現在のブラウザーに保存されます。GitHub でログインすると、アカウントのお気に入りと関連ソースコードをクラウドに同期できます。既存のローカル保存は自動でアップロードされません。既定の `http://127.0.0.1:43127/` 用の公開接続設定を同梱しているため、`.env` の設定は不要です。ネットワークによってクラウドに接続できない場合があり、VPN 不要の接続は保証しません。AI は利用者自身が設定し、料金を負担します。[詳細](docs/CLOUD_ACCOUNTS.md)。
