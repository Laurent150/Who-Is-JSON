...
function setup(){
 ...
 const context=vm.createContext({Event,AbortSignal,localStorage:storage,sessionStorage:storage,$:node,library(){},download(){},setTimeout:fn=>{timers.push(fn);return timers.length},clearTimeout(){},window:{APP_TOKEN:'local-token',dispatchEvent:e=>events.get(e.type)?.(),addEventListener:(k,v)=>events.set(k,v)},fetch:async(url,options)=>{
  const route=url.split('/').at(-1);calls.push({route,options});
  if(route==='status')return Response.json({enabled:true});
  if(route==='verify')return Response.json({session:'opaque'});
 ...
 return {nodes,node,data,calls,timers,store:context.window.WhoLibraryStore,finish:()=>pendingSave(),login:async()=>{node('accountEmail').value='a@example.com';node('accountCode').value='123456';await node('accountVerify').onclick();}};
}
...
test('initial cloud status enables login without switching the local library',async()=>{
 const s=setup();await tick();assert.equal(s.node('accountLogin').hidden,false);assert.equal(s.node('accountSend').disabled,false);assert.match(s.node('libraryLocation').textContent,/本地/);
});
