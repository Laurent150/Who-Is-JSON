(() => {
  ...
  const note = text => { $('accountStatus').textContent = text; };
  ...
  function refresh() {
    ...
    $('accountBtn').title = user ? '账户：' + user.email : '登录账户';
    $('libraryLocation').textContent = user ? '账户收藏：' + user.email + '。修改会同步到云端，包括收藏关联的源码。' : '本地收藏，仅保存在当前浏览器。登录后可使用独立的账户收藏库。';
    if ($('library').open) library();
  }
  function fail(error) {
    blocked = true;
    note(error.name === 'TimeoutError' ? '同步超时，本机修改已保留，可以重试。' : error.message);
    if (error.status === 401) note('登录已过期。本机修改已保留，请退出后重新登录。');
  }
  async function sync() {
  ...
  $('accountSend').onclick = () => action($('accountSend'), async () => {
    await call('send-code', { email: $('accountEmail').value });
    note('验证码已发送，请查看邮箱。首次验证成功会创建账户。');
  });
  $('accountVerify').onclick = () => action($('accountVerify'), async () => {
    const result = await call('verify', { email: $('accountEmail').value, code: $('accountCode').value.trim() });
    session = result.session;
    try { sessionStorage.setItem('who.account.session', session); } catch {}
    $('accountCode').value = ''; await enter();
  });
  ...
  call('status').then(async result => {
    $('accountSend').disabled = $('accountVerify').disabled = !result.enabled;
    if (!result.enabled) return note('此版本尚未配置云服务。你可以继续使用本地收藏，待云服务启用后再登录。');
    note('使用邮箱验证码登录；账户收藏会同步到云端，AI 设置保持本地。');
