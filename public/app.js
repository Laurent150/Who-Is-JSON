...
function connection(){
 ...
 $('connection').textContent=model?'● 已接入 '+model+(c.provider==='platform'?' · 赠送额度':''):'● 本地模式';
...
function saveCard(b){const cards=saved();cards.unshift({...b,start:b.start+sourceOffset,end:b.end+sourceOffset,id:Date.now(),language:current.language,name:fileName||'代码片段',saved:new Date().toLocaleString(),mode:current.mode});try{WhoLibraryStore.setItem('codelingo.cards',JSON.stringify(cards.slice(0,60)));toast('已收藏到「我的功能卡」。');}catch{toast('本地存储空间不足，请导出或删除部分收藏。');}}
...
$('settingsBtn').onclick=settings;$('saveSettings').onclick=()=>{config={base:$('base').value.trim().replace(/\/$/,''),model:$('model').value.trim(),key:$('key').value.trim()};if(!config.base||!config.model)return toast('请填写基础地址和模型名称。');try{const u=new URL(config.base);if(u.protocol!=='https:'&&!['localhost','127.0.0.1','[::1]'].includes(u.hostname))throw Error();}catch{return toast('请填写有效 HTTPS 地址，或本机 HTTP 地址。');}window.WhoTrialOptOut=true;localStorage.setItem('codelingo.config',JSON.stringify({base:config.base,model:config.model}));connection();if(current){renderStudio();renderTalk();}$('settings').close();toast('已配置。下一次 AI 操作会发送内容到此服务；连接尚未验证。');};$('disconnect').onclick=()=>{window.WhoTrialOptOut=true;config={};localStorage.removeItem('codelingo.config');connection();if(current){renderStudio();renderTalk();}$('settings').close();toast('已断开 AI 连接。');};
...
$('analyzeBtn').onclick=()=>run();$('selectionBtn').onclick=()=>run(true);$('clearBtn').onclick=()=>{removeImage();setCode('','');};const demo=()=>{removeImage();setCode(sample,'shopping_cart.py');run();};$('emptyDemo').onclick=demo;$('fileInput').onchange=e=>{loadFile(e.target.files[0]).catch(e=>toast(e.message));e.target.value='';};$('removeImage').onclick=removeImage;
...
$('widgetBtn').onclick=()=>task($('widgetBtn'),async()=>{await api('widget');toast('悬浮入口已打开，可截图或拖入文件。');});
