...
Who Is JSON 面向零基础学习者和使用 AI 写代码的人：导入别人的源码或 AI 生成的代码，先了解整体用途，再沿着函数、分支和循环对照原文，点开不懂的语句或符号继续学习。
...
本地解析支持 Python、JavaScript / TypeScript、Java、Bash、Dockerfile、JSON、YAML、HTML、CSS、SQL 和 `.gitignore`。**支持解析不等于能完整解释所有用法**；当前没有可靠的 C、C++、MATLAB 本地解析支持，也不能展开任意外部库或跨文件项目调用。
...
2. **查看本地结果。** 点击“查看整段结构”。默认进入“AI 代码工作台”；不使用 AI 时，可切到“结构资料”阅读本地结果。无法解析时先查看格式提示、语言和原文。
3. **连接 AI（按需）。** 在“设置”填写兼容 Chat Completions 的服务基础地址、模型和密钥。基础地址通常以 `/v1` 结尾，不是完整的 `/chat/completions` 地址。保存配置不代表连接已验证。
4. **沿流程学习。** 展开一个函数，点击“生成这个函数的 AI 流程”，再点步骤对照源码。点击代码行请求解释，`Shift` 连选；点击词语打开说明，`Escape` 关闭。方向键也可选行，右方向键进入词语。
5. **需要讲给别人听时，生成讲解稿。** 切换到“讲解稿 · 可选”，选择阅读基础、范围和详略，点击“生成 AI 讲解稿”。它会单独生成，不要求先生成整体 AI 分析。
